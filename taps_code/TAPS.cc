...
vector<struct Sched_Elem> traffic_sched(vector<struct Task_elem> task, int time_start) { 
	// time_start: the absolute starting time for the current schedule cycle 
	vector<struct Sched_Elem> sched;
	for(int i; i < task.size(); i++) {
		struct tmp_sched_task_elem;
		for(int k = 0; k < task[i].flow_num; k++){
			string link_id = to_str(task[i].flows[k].src) + "_" + to_str(task[i].flows[k].dst);
			if(task[i].flows[k].size / LinkCapa + (Link_state[link_id] - time_start) > deadline) { //can't complete this task!
...
//server side
int main() {
	[i]  = thread(&Transmitter::send_td_func,
